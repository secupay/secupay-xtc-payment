...
class secupay_ls_xtc
{
    ...
    public function __construct()
    {
        ...
        $this->title = (function_exists('xtc_catalog_href_link')) ? MODULE_PAYMENT_SPLS_TEXT_TITLE . "<br/><img style='border:0;text-decoration:none;' alt='secupay logo' src='https://www.secupay.ag/sites/default/files/media/Icons/secupay_logo.png'/>" : MODULE_PAYMENT_SPLS_TEXT_TITLE;
        ...
        if (defined('MODULE_PAYMENT_SPLS_STATUS')) {
            if (MODULE_PAYMENT_SPLS_STATUS == 'JA') {
                define('MODULE_PAYMENT_SPLS_STATUS', true);
            }
            if (MODULE_PAYMENT_SPLS_STATUS == 'Nein') {
                define('MODULE_PAYMENT_SPLS_STATUS', false);
            }
            xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SECUPAY_LS_XTC_STATUS', '" . MODULE_PAYMENT_SPLS_STATUS . "', '6', '0', 'xtc_cfg_select_option(array(\'true\', \'false\'), ', now())");
            xtc_db_query("DELETE FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_SPLS_STATUS'");
        ...
        $this->enabled = strcmp(MODULE_PAYMENT_SECUPAY_LS_XTC_STATUS, 'true') == 0;
        $this->sp_log = strcmp(MODULE_PAYMENT_SPLS_LOGGING, 'Ja') == 0;
        ...
        secupay_log($this->sp_log, "Aktiv: " . MODULE_PAYMENT_SECUPAY_LS_XTC_STATUS);
        ...
        if ($this->order_before_payment) {
            secupay_log($this->sp_log, "secupay_ls_xtc - order_before_payment activ");
            $this->tmpOrders = true;
            $this->tmpStatus = MODULE_PAYMENT_SPLS_ORDER_STATUS_ID;
            //for xtc
    ...
    public function update_status()
    {
        ...
        if (($this->enabled) && ((int)MODULE_PAYMENT_SPLS_ZONE > 0)) {
            $check_flag = false;
            try {
                $check_query = xtc_db_query("SELECT geo_zone_id FROM " . TABLE_ZONES_TO_GEO_ZONES . " WHERE geo_zone_id = '" . MODULE_PAYMENT_SPLS_ZONE . "' AND zone_country_id = '" . $order->billing['country']['id'] . "' ORDER BY zone_id");
                while ($check = xtc_db_fetch_array($check_query)) {
                    if ($check['geo_zone_id'] == MODULE_PAYMENT_SPLS_ZONE) {
                        $check_flag = true;
                        break;
                    }
                }
        ...
        if (($this->enabled) && $this->conf->secupay_ls_xtc->delivery_differs_disable && MODULE_PAYMENT_SPLS_DELIVERY_DISABLE == "Ja") {
        ...
        if (($this->enabled) && is_numeric(MODULE_PAYMENT_SPLS_MIN_AMOUNT) && MODULE_PAYMENT_SPLS_MIN_AMOUNT > 0) {
            secupay_log($this->sp_log, "secupay_ls_xtc - update_status - check amount");
            ...
            $check_amount = number_format(floatval(MODULE_PAYMENT_SPLS_MIN_AMOUNT), 2, '.', '');

            secupay_log($this->sp_log, "secupay_ls_xtc - update_status - order amount: " . $order_amount . " - amount set: " . $check_amount);

            if ($order_amount < $check_amount) {
                $this->enabled = false;
                secupay_log($this->sp_log, "secupay_ls_xtc - update_status - disabled because of low amount");
            }
        }
        if (($this->enabled) && is_numeric(MODULE_PAYMENT_SPLS_MAX_AMOUNT) && MODULE_PAYMENT_SPLS_MAX_AMOUNT > 0) {
            secupay_log($this->sp_log, "secupay_ls_xtc - update_status - check for max amount");
            ...
            $check_amount = number_format(floatval(MODULE_PAYMENT_SPLS_MAX_AMOUNT), 2, '.', '');

            secupay_log($this->sp_log, "secupay_ls_xtc - update_status - order amount: " . $order_amount . " - amount set: " . $check_amount);

            if ($order_amount >= $check_amount) {
                $this->enabled = false;
                secupay_log($this->sp_log, "secupay_ls_xtc - update_status - disabled because of high amount");
            }
        }
    }
    ...
    public function selection()
    {
        ...
        if ($order->info['currency'] == 'EUR') {
            ...
            $logo = "<img alt=\"" . MODULE_PAYMENT_SPLS_TEXT_DESCRIPTION . "\" style=\"border:0;margin: -9px -9px -18px -2px;\" src=\"https://www.secupay.ag/sites/default/files/media/Icons/" . $lang_logo . "secupay_debit.png\" align=\"middle\" height=\"38px\"  />";
            $title = $logo;
            ...
            $selection['description'] = "<br />" . MODULE_PAYMENT_SECUPAY_SPLS_TEXT_INFO;
    ...
    public function check()
    {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_SECUPAY_LS_XTC_STATUS'");
    ...
    public function before_process()
    {
        ...
        if (!isset($_SESSION['sp_success'])) {
            ...
            $data['url_success'] = str_replace(
                '&amp;',
                '&',
                xtc_href_link('secupay_checkout_success.php', 'payment_error=secupay_ls_xtc&uid=' . $uid, 'SSL', true, false)
            );
            ...
            $data["url_push"] = xtc_href_link(
                ...
                'payment_type=' . $this->conf->secupay_ls_xtc->payment_type,
                'SSL',
                true,
                false
            );
            ...
            $module_config->add_tax_for_shipping = MODULE_PAYMENT_SPLS_MWST;
    ...
    public function payment_action()
    {
        ...
        secupay_log($this->sp_log, 'secupay_ls_xtc - payment_action called');
        if (!$this->order_before_payment) {
            return;
        }
        ...
        //Kompatibilität mit gambio
        $_SESSION['sp_tag'] = 1;
        secupay_log($this->sp_log, 'secupay_ls_xtc - payment_action redirect');
        xtc_redirect(xtc_href_link("secupay_checkout_iframe.php", "", 'SSL', true, false));
    }
    ...
    public function install()
    {
        ...
        $experience = $this->conf->secupay_ls_xtc->experience ? 'Ja' : 'Nein';
        $tautosend = $this->conf->secupay_ls_xtc->tautosend ? 'Ja' : 'Nein';

        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SECUPAY_LS_XTC_STATUS', 'true', '6', '0', 'xtc_cfg_select_option(array(\'true\', \'false\'), ', now())");
        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPLS_SORT_ORDER', '0',  '6', '0' , now())");
        if (!defined('MODULE_PAYMENT_SECUPAY_APIKEY')) {
            xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SECUPAY_APIKEY', '{$vertrag_id_ls}',  '6', '0' , now()) ON DUPLICATE KEY UPDATE configuration_value = '{$vertrag_id_ls}'");
        }
        ...
        if (!defined('MODULE_PAYMENT_SPLS_DELIVERY_DISABLE')) {
        ...
        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPLS_MIN_AMOUNT', '0.00',  '6', '0' , now())");

        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPLS_MAX_AMOUNT', '0.00',  '6', '0' , now())");
    ...
    public function remove()
    {
        ...
        xtc_db_query("DELETE FROM " . TABLE_CONFIGURATION . " WHERE configuration_key IN ('" . implode(
            "', '",
                $this->keys()
        ) . "','MODULE_PAYMENT_SECUPAY_LS_XTC_ALLOWED') AND configuration_key NOT IN ('MODULE_PAYMENT_SECUPAY_APIKEY','MODULE_PAYMENT_SECUPAY_EXPERIENCE','MODULE_PAYMENT_SECUPAY_TAUTOSEND','MODULE_PAYMENT_SECUPAY_ORDER_BEFORE_PAYMENT','MODULE_PAYMENT_SECUPAY_SESSION')");
    ...
    {
        $keys = array(
            'MODULE_PAYMENT_SECUPAY_LS_XTC_STATUS',
            ...
            'MODULE_PAYMENT_SPLS_MIN_AMOUNT',
            'MODULE_PAYMENT_SPLS_MAX_AMOUNT',
