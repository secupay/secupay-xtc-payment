...
/**
 * class that provides access to Secupay Sofortüberweiung payments
 */
class secupay_sk_xtc
{
    /**
     * @var string
     */
    ...
    public function __construct()
    {
        ...
        $this->code = 'secupay_sk_xtc';
        $this->title = (function_exists('xtc_catalog_href_link')) ? MODULE_PAYMENT_SPSK_TEXT_TITLE . "<br/><img style='border:0;text-decoration:none;' alt='secupay logo' src='https://www.secupay.ag/sites/default/files/media/Icons/secupay_logo.png'/>" : MODULE_PAYMENT_SPSK_TEXT_TITLE;
        $this->sort_order = MODULE_PAYMENT_SPSK_SORT_ORDER;
        $pref = "INSERT INTO " . TABLE_CONFIGURATION . " ( configuration_key, configuration_value,  configuration_group_id, sort_order,";
        if (defined('MODULE_PAYMENT_SPSK_STATUS')) {
            if (MODULE_PAYMENT_SPSK_STATUS == 'JA') {
                define('MODULE_PAYMENT_SPSK_STATUS', true);
            }
            if (MODULE_PAYMENT_SPSK_STATUS == 'Nein') {
                define('MODULE_PAYMENT_SPSK_STATUS', false);
            }
            xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SECUPAY_SK_XTC_STATUS', '" . MODULE_PAYMENT_SPSK_STATUS . "', '6', '0', 'xtc_cfg_select_option(array(\'true\', \'false\'), ', now())");
            xtc_db_query("DELETE FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_SPSK_STATUS'");
        ...
        $this->enabled = strcmp(MODULE_PAYMENT_SECUPAY_SK_XTC_STATUS, 'true') == 0;
        $this->sp_log = strcmp(MODULE_PAYMENT_SPSK_LOGGING, 'Ja') == 0;
        ...
        //Demo Modus
        if (strcmp(MODULE_PAYMENT_SPSK_SIMULATION_MODE, "Simulation") == 0) {
            $this->demo = true;
        } else {
            $this->demo = false;
        }

        if ((int)MODULE_PAYMENT_SPSK_ORDER_STATUS_ID > 0) {
            $this->order_status_success = MODULE_PAYMENT_SPSK_ORDER_STATUS_ID;
        }
        $this->conf = get_sp_conf();

        $this->description = MODULE_PAYMENT_SPSK_TEXT_DESCRIPTION . "<br/>Version " . $this->conf->general->modulversion .
        ...
        $this->description .= '<iframe frameborder="0" scrolling="auto" src="' . SECUPAY_INFO_IFRAME_URL . '?vertrag_id_sk=' . MODULE_PAYMENT_SECUPAY_APIKEY . '&amp;shop=' . $this->conf->general->shop . '&amp;shopversion=' . urlencode(PROJECT_VERSION) . '&amp;modulversion=' . urlencode($this->conf->general->modulversion) . '" marginwidth="0px" marginheight="0px" name="secupay_info" style="border:0px; display:block;width:98%;margin-top:3px;margin-bottom:3px;"></iframe>';

        secupay_log($this->sp_log, "secupay_sk_xtc constructor php " . PHP_VERSION);
        secupay_log($this->sp_log, "Conf: " . json_encode($this->conf));
        secupay_log($this->sp_log, "Modus: " . MODULE_PAYMENT_SPSK_SIMULATION_MODE);
        secupay_log($this->sp_log, "Sort Order: " . MODULE_PAYMENT_SPSK_SORT_ORDER);
        secupay_log($this->sp_log, "Aktiv: " . MODULE_PAYMENT_SECUPAY_SK_XTC_STATUS);
        secupay_log($this->sp_log, "Shopname: " . MODULE_PAYMENT_SPSK_SHOPNAME);
        secupay_log($this->sp_log, "Order Status erfolg: " . MODULE_PAYMENT_SPSK_ORDER_STATUS_ID);
        secupay_log($this->sp_log, "Order Status accepted: " . MODULE_PAYMENT_SPSK_ORDER_STATUS_ACCEPTED_ID);
        secupay_log($this->sp_log, "Order Status denied: " . MODULE_PAYMENT_SPSK_ORDER_STATUS_DENIED_ID);
        secupay_log($this->sp_log, "Order Status issue: " . MODULE_PAYMENT_SPSK_ORDER_STATUS_ISSUE_ID);
        secupay_log($this->sp_log, "Order Status void: " . MODULE_PAYMENT_SPSK_ORDER_STATUS_VOID_ID);
        secupay_log($this->sp_log, "Order Status authorized: " . MODULE_PAYMENT_SPSK_ORDER_STATUS_AUTHORIZED_ID);

        if ($this->order_before_payment) {
            secupay_log($this->sp_log, "secupay_sk_xtc - order_before_payment activ");
            $this->tmpOrders = true;
            $this->tmpStatus = MODULE_PAYMENT_SPSK_ORDER_STATUS_ID;
            //for xtc
    ...
    public function selection()
    {
        ...
        if ($order->info['currency'] == 'EUR') {
            ...
            $logo = "<img alt=\"" . MODULE_PAYMENT_SPSK_TEXT_DESCRIPTION . "\" style=\"border:0;margin: -9px -9px -18px -2px;\" src=\"https://cdn.klarna.com/1.0/shared/image/generic/badge/" . $lang_logo . "pay_now/standard/pink.svg\" align=\"middle\" height=\"38px\" />";
            $title = $logo;
            ...
            $selection['description'] = "<br />" . MODULE_PAYMENT_SECUPAY_SPSK_TEXT_INFO;
    ...
    public function check()
    {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_SECUPAY_SK_XTC_STATUS'");
    ...
    public function confirmation()
    {
        ...
        if ($this->conf->secupay_sk_xtc->delivery_differs_disable && MODULE_PAYMENT_SPSK_DELIVERY_DISABLE == "Hinweis" && $this->_check_delivery_differs($order)) {
            $additional_messages = array('title' => MODULE_PAYMENT_SPSK_HINT, 'field' => MODULE_PAYMENT_SPSK_DELIVERY_HINT);
    ...
    public function process_button()
    {
        ...
        $_SESSION['secupay_sk_total'] = number_format($total, 2, '.', '');
    ...
    public function before_process()
    {
        ...
        if (!isset($_SESSION['sp_success'])) {
            ...
            $store_name = MODULE_PAYMENT_SPSK_SHOPNAME;
            ...
            $data['url_success'] = str_replace(
                '&amp;',
                '&',
                xtc_href_link('secupay_checkout_success.php', 'payment_error=secupay_sk_xtc&uid=' . $uid, 'SSL', true, false)
            );
            $data['url_failure'] = xtc_href_link('secupay_checkout_failure.php', 'payment_error=secupay_sk_xtc', 'SSL', true, false);
            ...
            $data["payment_type"] = $this->conf->secupay_sk_xtc->payment_type;
            $data["payment_action"] = $this->conf->secupay_sk_xtc->payment_action;
            $data["url_push"] = xtc_href_link(
                'secupay_status_push.php',
                'payment_type=' . $this->conf->secupay_sk_xtc->payment_type,
                'SSL',
                true,
                false
            );
            ...
            $module_config->add_tax_for_shipping = MODULE_PAYMENT_SPSK_MWST;
            ...
            unset($_SESSION['secupay_sk_total']);
            ...
            if ($api_return->status != "ok") { //Fehler
                ...
                if (!$this->order_before_payment) {
                    //Kompatibilität mit gambio
                    $_SESSION['sp_tag'] = 1;
                    xtc_redirect($api_return->data->iframe_url);
                }
            }
        }
    ...
    public function payment_action()
    {
        ...
        secupay_log($this->sp_log, 'secupay_sk_xtc - payment_action called');
        if (!$this->order_before_payment) {
            return;
        }
        ...
        //Kompatibilität mit gambio
        $_SESSION['sp_tag'] = 1;
        secupay_log($this->sp_log, 'secupay_sk_xtc - payment_action redirect');
        xtc_redirect($_SESSION['iframe_link']);
    }
    ...
    public function after_process()
    {
        global $insert_id;
        $is_demo = $this->demo;
        $is_delivery_differs = isset($_SESSION['sp_sk_delivery_differs']) && $_SESSION['sp_sk_delivery_differs'];
        ...
        if ($is_demo) {
            $comment .= MODULE_PAYMENT_SPSK_DEMO_HINT;
        }
    ...
    public function get_error()
    {
        ...
        $error = array('title' => MODULE_PAYMENT_SPSK_TEXT_ERROR, 'error' => stripslashes(urldecode($fehler)));
        return $error;
    }

    /**
     * Function that installs Secupay Sofort payment module
     */
    public function install()
    {
        ...
        if ($this->conf->general->apikey && is_string($this->conf->general->apikey)) {
            $vertrag_id_sk = trim($this->conf->general->apikey);
        }

        $modus = $this->conf->secupay_sk_xtc->modus == "live" ? "Produktivsystem" : "Simulation";
        $debug = $this->conf->secupay_sk_xtc->debug ? 'Ja' : 'Nein';
        $lw = $this->conf->secupay_sk_xtc->lieferanschrift_warnung ? 'Ja' : 'Nein';
        $modul = $this->conf->general->modul;
        $experience = $this->conf->secupay_sk_xtc->experience ? 'Ja' : 'Nein';
        $tautosend = $this->conf->secupay_sk_xtc->tautosend ? 'Ja' : 'Nein';

        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SECUPAY_SK_XTC_STATUS', 'true', '6', '0', 'xtc_cfg_select_option(array(\'true\', \'false\'), ', now())");
        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPSK_SORT_ORDER', '0',  '6', '0' , now())");
        if (!defined('MODULE_PAYMENT_SECUPAY_APIKEY')) {
            xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SECUPAY_APIKEY', '{$vertrag_id_sk}',  '6', '0' , now()) ON DUPLICATE KEY UPDATE configuration_value = '{$vertrag_id_sk}'");
        }
        ...
        xtc_db_query("$pref use_function, set_function, date_added) values ( 'MODULE_PAYMENT_SPSK_ZONE', '0',  '6', '0', 'xtc_get_zone_class_title', 'xtc_cfg_pull_down_zone_classes(', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPSK_ORDER_STATUS_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");

        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPSK_ORDER_STATUS_ACCEPTED_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPSK_ORDER_STATUS_DENIED_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPSK_ORDER_STATUS_ISSUE_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPSK_ORDER_STATUS_VOID_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPSK_ORDER_STATUS_AUTHORIZED_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");

        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPSK_SIMULATION_MODE', '$modus', '6', '0', 'xtc_cfg_select_option(array(\'Simulation\', \'Produktivsystem\'), ', now())");
        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPSK_MWST', 'Nein', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");
        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPSK_LOGGING','$debug', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");
        ...
        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPSK_SHOPNAME', '',  '6', '0' , now())");
        //xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPSK_PREAUTH', '$aut', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");
        //xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPSK_BID', 'Ja', '6', '99', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");
        ...
        if (!defined('MODULE_PAYMENT_SPSK_DELIVERY_DISABLE')) {
            xtc_db_query("$pref set_function, date_added) values ('MODULE_PAYMENT_SPSK_DELIVERY_DISABLE', 'Nein', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\', \'Hinweis\'), ', now())");
        }

        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SECUPAY_SK_XTC_ALLOWED', '', '6', '0', now())");

        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPSK_MIN_AMOUNT', '0.00',  '6', '0' , now())");

        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPSK_MAX_AMOUNT', '0.00',  '6', '0' , now())");
        ...
        //xtc_db_query("DROP TABLE IF EXISTS secupay_history_sk");
        xtc_db_query(
    ...
    {
        $keys = array(
            'MODULE_PAYMENT_SECUPAY_SK_XTC_STATUS',
            'MODULE_PAYMENT_SECUPAY_APIKEY',
            'MODULE_PAYMENT_SPSK_SORT_ORDER',
            'MODULE_PAYMENT_SPSK_SHOPNAME',
            'MODULE_PAYMENT_SPSK_SIMULATION_MODE',
            'MODULE_PAYMENT_SPSK_LOGGING',
            'MODULE_PAYMENT_SECUPAY_EXPERIENCE',
            'MODULE_PAYMENT_SECUPAY_TAUTOSEND',
            'MODULE_PAYMENT_SPSK_ZONE',
            'MODULE_PAYMENT_SPSK_ORDER_STATUS_ID',
            'MODULE_PAYMENT_SPSK_ORDER_STATUS_ACCEPTED_ID',
            'MODULE_PAYMENT_SPSK_ORDER_STATUS_DENIED_ID',
            'MODULE_PAYMENT_SPSK_ORDER_STATUS_ISSUE_ID',
            'MODULE_PAYMENT_SPSK_ORDER_STATUS_VOID_ID',
            'MODULE_PAYMENT_SPSK_ORDER_STATUS_AUTHORIZED_ID',
            'MODULE_PAYMENT_SPSK_MIN_AMOUNT',
            'MODULE_PAYMENT_SPSK_MAX_AMOUNT',
            'MODULE_PAYMENT_SPSK_MWST',
            'MODULE_PAYMENT_SECUPAY_ORDER_BEFORE_PAYMENT',
            'MODULE_PAYMENT_SECUPAY_SESSION'
        );

        if ($this->conf->secupay_sk_xtc->delivery_differs_disable) {
            $keys[] = 'MODULE_PAYMENT_SPSK_DELIVERY_DISABLE';
        }
