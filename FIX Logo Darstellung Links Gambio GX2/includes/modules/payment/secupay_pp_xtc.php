...
/**
 * class that provides access to Secupay Prepay payments
 */
class secupay_pp_xtc
{
    /**
     * @var string
     */
    ...
    public function __construct()
    {
        ...
        $this->code = 'secupay_pp_xtc';
        $this->title = (function_exists('xtc_catalog_href_link')) ? MODULE_PAYMENT_SPPP_TEXT_TITLE . "<br/><img style='border:0;text-decoration:none;' alt='secupay logo' src='https://www.secupay.ag/sites/default/files/media/Icons/secupay_logo.png'/>" : MODULE_PAYMENT_SPPP_TEXT_TITLE;
        $this->sort_order = MODULE_PAYMENT_SPPP_SORT_ORDER;
        $pref = "INSERT INTO " . TABLE_CONFIGURATION . " ( configuration_key, configuration_value,  configuration_group_id, sort_order,";
        if (defined(MODULE_PAYMENT_SPPP_STATUS)) {
            if (MODULE_PAYMENT_SPPP_STATUS == 'JA') {
                define('MODULE_PAYMENT_SPPP_STATUS', true);
            }
            if (MODULE_PAYMENT_SPPP_STATUS == 'Nein') {
                define('MODULE_PAYMENT_SPPP_STATUS', false);
            }
            xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SECUPAY_PP_XTC_STATUS', '" . MODULE_PAYMENT_SPPP_STATUS . "', '6', '0', 'xtc_cfg_select_option(array(\'true\', \'false\'), ', now())");
            xtc_db_query("DELETE FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_SPPP_STATUS'");
        ...
        $this->enabled = strcmp(MODULE_PAYMENT_SECUPAY_PP_XTC_STATUS, 'true') == 0;
        $this->experience = strcmp(MODULE_PAYMENT_SECUPAY_EXPERIENCE, 'Ja') == 0;
        $this->tautosend = strcmp(MODULE_PAYMENT_SECUPAY_TAUTOSEND, 'Ja') == 0;
        $this->sp_log = strcmp(MODULE_PAYMENT_SPPP_LOGGING, 'Ja') == 0;
        $this->order_before_payment = strcmp(MODULE_PAYMENT_SECUPAY_ORDER_BEFORE_PAYMENT, 'Ja') == 0;

        //Demo Modus
        if (strcmp(MODULE_PAYMENT_SPPP_SIMULATION_MODE, "Simulation") == 0) {
            $this->demo = true;
        } else {
            $this->demo = false;
        }

        if ((int)MODULE_PAYMENT_SPPP_ORDER_STATUS_ID > 0) {
            $this->order_status_success = MODULE_PAYMENT_SPPP_ORDER_STATUS_ID;
        }
        $this->conf = get_sp_conf();

        $this->description = MODULE_PAYMENT_SPPP_TEXT_DESCRIPTION . "<br/>Version " . $this->conf->general->modulversion .
        ...
        secupay_log($this->sp_log, "secupay_pp_xtc constructor php " . PHP_VERSION);
        secupay_log($this->sp_log, "Conf: " . json_encode($this->conf));
        secupay_log($this->sp_log, "Modus: " . MODULE_PAYMENT_SPPP_SIMULATION_MODE);
        secupay_log($this->sp_log, "Sort Order: " . MODULE_PAYMENT_SPPP_SORT_ORDER);
        secupay_log($this->sp_log, "Aktiv: " . MODULE_PAYMENT_SECUPAY_PP_XTC_STATUS);
        secupay_log($this->sp_log, "Shopname: " . MODULE_PAYMENT_SPPP_SHOPNAME);
        secupay_log($this->sp_log, "Order Status erfolg: " . MODULE_PAYMENT_SPPP_ORDER_STATUS_ID);
        secupay_log($this->sp_log, "Order Status accepted: " . MODULE_PAYMENT_SPPP_ORDER_STATUS_ACCEPTED_ID);
        secupay_log($this->sp_log, "Order Status denied: " . MODULE_PAYMENT_SPPP_ORDER_STATUS_DENIED_ID);
        secupay_log($this->sp_log, "Order Status issue: " . MODULE_PAYMENT_SPPP_ORDER_STATUS_ISSUE_ID);
        secupay_log($this->sp_log, "Order Status void: " . MODULE_PAYMENT_SPPP_ORDER_STATUS_VOID_ID);
        secupay_log($this->sp_log, "Order Status authorized: " . MODULE_PAYMENT_SPPP_ORDER_STATUS_AUTHORIZED_ID);

        if ($this->order_before_payment) {
            secupay_log($this->sp_log, "secupay_pp_xtc - order_before_payment activ");
            $this->tmpOrders = true;
            $this->tmpStatus = MODULE_PAYMENT_SPPP_ORDER_STATUS_ID;
            //for xtc
    ...
    public function update_status()
    {
        ...
        if (($this->enabled) && ((int)MODULE_PAYMENT_SPPP_ZONE > 0)) {
            $check_flag = false;
            try {
                $check_query = xtc_db_query("SELECT geo_zone_id FROM " . TABLE_ZONES_TO_GEO_ZONES . " WHERE geo_zone_id = '" . MODULE_PAYMENT_SPPP_ZONE . "' AND zone_country_id = '" . $order->billing['country']['id'] . "' ORDER BY zone_id");
                while ($check = xtc_db_fetch_array($check_query)) {
                    if ($check['geo_zone_id'] ==MODULE_PAYMENT_SPPP_ZONE) {
                        $check_flag = true;
                        break;
                    }
                }
        ...
        if (($this->enabled) && $this->conf->secupay_pp_xtc->delivery_differs_disable && MODULE_PAYMENT_SPPP_DELIVERY_DISABLE == "Ja") {
        ...
        if (($this->enabled) && is_numeric(MODULE_PAYMENT_SPPP_MIN_AMOUNT) && MODULE_PAYMENT_SPPP_MIN_AMOUNT > 0) {
            secupay_log($this->sp_log, "secupay_pp_xtc - update_status - check for min amount");
            ...
            $check_amount = number_format(floatval(MODULE_PAYMENT_SPPP_MIN_AMOUNT), 2, '.', '');

            secupay_log($this->sp_log, "secupay_pp_xtc - update_status - order amount: " . $order_amount . " - amount set: " . $check_amount);

            if ($order_amount < $check_amount) {
                $this->enabled = false;
                secupay_log($this->sp_log, "secupay_pp_xtc - update_status - disabled because of low amount");
            }
        }
        if (($this->enabled) && is_numeric(MODULE_PAYMENT_SPPP_MAX_AMOUNT) && MODULE_PAYMENT_SPPP_MAX_AMOUNT > 0) {
            secupay_log($this->sp_log, "secupay_pp_xtc - update_status - check for max amount");
            ...
            $check_amount = number_format(floatval(MODULE_PAYMENT_SPPP_MAX_AMOUNT), 2, '.', '');

            secupay_log($this->sp_log, "secupay_pp_xtc - update_status - order amount: " . $order_amount . " - amount set: " . $check_amount);

            if ($order_amount >= $check_amount) {
                $this->enabled = false;
                secupay_log($this->sp_log, "secupay_pp_xtc - update_status - disabled because of high amount");
            }
        }
    }
    ...
    public function selection()
    {
        ...
        if ($order->info['currency'] == 'EUR') {
            ...
            $logo = "<img alt=\"" . MODULE_PAYMENT_SPPP_TEXT_DESCRIPTION . "\" style=\"border:0;margin: -9px -9px -18px -2px;\" src=\"https://www.secupay.ag/sites/default/files/media/Icons/" . $lang_logo . "secupay_prepay.png\" align=\"middle\" height=\"38px\" />";
            $title = $logo;
            ...
            $selection['description'] = "<br />" . MODULE_PAYMENT_SECUPAY_SPPP_TEXT_INFO;
    ...
    public function check()
    {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_SECUPAY_PP_XTC_STATUS'");
    ...
    public function confirmation()
    {
        ...
        if ($this->conf->secupay_pp_xtc->delivery_differs_disable && MODULE_PAYMENT_SPPP_DELIVERY_DISABLE == "Hinweis" && $this->_check_delivery_differs($order)) {
            $additional_messages = array('title' => MODULE_PAYMENT_SPPP_HINT, 'field' => MODULE_PAYMENT_SPPP_DELIVERY_HINT);
    ...
    public function before_process()
    {
        ...
        if (!isset($_SESSION['sp_success'])) {
            ...
            $_mwst_bug = 0;
            if (MODULE_PAYMENT_SPPP_MWST == "Ja") {
            ...
            $store_name = MODULE_PAYMENT_SPPP_SHOPNAME;
            ...
            $data['url_success'] = str_replace(
                '&amp;',
                '&',
                xtc_href_link('secupay_checkout_success.php', 'payment_error=secupay_pp_xtc&uid=' . $uid, 'SSL', true, false)
            );
            $data['url_failure'] = xtc_href_link('secupay_checkout_failure.php', 'payment_error=secupay_pp_xtc', 'SSL', true, false);
            ...
            $data["payment_type"] = $this->conf->secupay_pp_xtc->payment_type;
            $data["payment_action"] = $this->conf->secupay_pp_xtc->payment_action;
            $data["url_push"] = xtc_href_link(
                'secupay_status_push.php',
                'payment_type=' . $this->conf->secupay_pp_xtc->payment_type,
                'SSL',
                true,
                false
            );
            ...
            $module_config->add_tax_for_shipping = MODULE_PAYMENT_SPPP_MWST;
    ...
    public function payment_action()
    {
        ...
        secupay_log($this->sp_log, 'secupay_pp_xtc - payment_action called');
        if (!$this->order_before_payment) {
            return;
        }
        ...
        //Kompatibilität mit gambio
        $_SESSION['sp_tag'] = 1;
        secupay_log($this->sp_log, 'secupay_pp_xtc - payment_action redirect');
        xtc_redirect(xtc_href_link("secupay_checkout_iframe.php", "", 'SSL', true, false));
    }
    ...
    public function after_process()
    {
        ...
        if ($is_demo) {
            $comment .= MODULE_PAYMENT_SPPP_DEMO_HINT;
        }
        ...
        try {
            ...
            $sql = "UPDATE " . TABLE_ORDERS . " SET orders_status='" . $this->order_status_success . "' WHERE orders_id='" . $insert_id . "'";
            secupay_log($this->sp_log, $sql);
            xtc_db_query($sql);
            //add payment info to order comment
            if (MODULE_PAYMENT_SPPP_PAYMENTINFO_TO_COMMENT != "Nein") {
                ...
                if (xtc_db_num_rows($qry) == 1) {
                    ...
                    if (MODULE_PAYMENT_SPPP_PAYMENTINFO_TO_COMMENT == 'Ja (TABLE_ORDERS)') {
    ...
    public function get_error()
    {
        ...
        $error = array('title' => MODULE_PAYMENT_SPPP_TEXT_ERROR, 'error' => stripslashes(urldecode($fehler)));
        return $error;
    }
    ...
    public function install()
    {
        ...
        if ($this->conf->general->apikey && is_string($this->conf->general->apikey)) {
            $vertrag_id_pp = trim($this->conf->general->apikey);
        }

        $modus = $this->conf->secupay_pp_xtc->modus == "live" ? "Produktivsystem" : "Simulation";
        $debug = $this->conf->secupay_pp_xtc->debug ? 'Ja' : 'Nein';
        $experience = $this->conf->secupay_pp_xtc->experience ? 'Ja' : 'Nein';
        $tautosend = $this->conf->secupay_pp_xtc->tautosend ? 'Ja' : 'Nein';
        $lw = $this->conf->secupay_pp_xtc->lieferanschrift_warnung ? 'Ja' : 'Nein';
        $modul = $this->conf->general->modul;

        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SECUPAY_PP_XTC_STATUS', 'true', '6', '0', 'xtc_cfg_select_option(array(\'true\', \'false\'), ', now())");
        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPPP_SORT_ORDER', '0',  '6', '0' , now())");
        if (!defined('MODULE_PAYMENT_SECUPAY_APIKEY')) {
            xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SECUPAY_APIKEY', '{$vertrag_id_pp}',  '6', '0' , now()) ON DUPLICATE KEY UPDATE configuration_value = '{$vertrag_id_pp}'");
        }
        ...
        xtc_db_query("$pref use_function, set_function, date_added) values ( 'MODULE_PAYMENT_SPPP_ZONE', '0',  '6', '0', 'xtc_get_zone_class_title', 'xtc_cfg_pull_down_zone_classes(', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPPP_ORDER_STATUS_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPPP_ORDER_STATUS_ACCEPTED_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPPP_ORDER_STATUS_DENIED_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPPP_SIMULATION_MODE', '$modus', '6', '0', 'xtc_cfg_select_option(array(\'Simulation\', \'Produktivsystem\'), ', now())");
        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPPP_MWST', 'Nein', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");
        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPPP_LOGGING','$debug', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");
        ...
        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPPP_SHOW_QRCODE','Ja', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");

        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPPP_DUE_DATE','Nein', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");

        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPPP_SHOPNAME', '',  '6', '0' , now())");
        ...
        if (!defined('MODULE_PAYMENT_SPPP_DELIVERY_DISABLE')) {
            xtc_db_query("$pref set_function, date_added) values ('MODULE_PAYMENT_SPPP_DELIVERY_DISABLE', 'Nein', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\', \'Hinweis\'), ', now())");
        }

        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPPP_PAYMENTINFO_TO_COMMENT', 'Nein', '6', '0', 'xtc_cfg_select_option(array(\'Ja (TABLE_ORDERS)\', \'Ja (TABLE_ORDERS_STATUS_HISTORY)\', \'Nein\'), ', now())");

        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SECUPAY_PP_XTC_ALLOWED', '', '6', '0', now())");

        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPPP_MIN_AMOUNT', '0.00',  '6', '0' , now())");
        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPPP_MAX_AMOUNT', '0.00',  '6', '0' , now())");
    ...
    public function remove()
    {
        ...
        xtc_db_query("DELETE FROM " . TABLE_CONFIGURATION . " WHERE configuration_key IN ('" . implode(
            "', '",
                $this->keys()
        ) . "','MODULE_PAYMENT_SECUPAY_PP_XTC_ALLOWED') AND configuration_key NOT IN ('MODULE_PAYMENT_SECUPAY_APIKEY','MODULE_PAYMENT_SECUPAY_EXPERIENCE','MODULE_PAYMENT_SECUPAY_TAUTOSEND','MODULE_PAYMENT_SECUPAY_ORDER_BEFORE_PAYMENT','MODULE_PAYMENT_SECUPAY_SESSION')");
     ...
     *
     * @return array
     * 'MODULE_PAYMENT_SPPP_SHOW_QRCODE',
     */
    public function keys()
    {
        $keys = array(
            'MODULE_PAYMENT_SECUPAY_PP_XTC_STATUS',
            'MODULE_PAYMENT_SECUPAY_APIKEY',
            'MODULE_PAYMENT_SPPP_SORT_ORDER',
            'MODULE_PAYMENT_SPPP_SHOPNAME',
            'MODULE_PAYMENT_SPPP_SIMULATION_MODE',
            'MODULE_PAYMENT_SPPP_LOGGING',
            'MODULE_PAYMENT_SECUPAY_EXPERIENCE',
            'MODULE_PAYMENT_SECUPAY_TAUTOSEND',
            'MODULE_PAYMENT_SPPP_ZONE',
            'MODULE_PAYMENT_SPPP_ORDER_STATUS_ID',
            'MODULE_PAYMENT_SPPP_ORDER_STATUS_ACCEPTED_ID',
            'MODULE_PAYMENT_SPPP_ORDER_STATUS_DENIED_ID',
            'MODULE_PAYMENT_SPPP_MWST',
            'MODULE_PAYMENT_SPPP_DUE_DATE',
            'MODULE_PAYMENT_SPPP_MIN_AMOUNT',
            'MODULE_PAYMENT_SPPP_MAX_AMOUNT',
            'MODULE_PAYMENT_SPPP_PAYMENTINFO_TO_COMMENT',
            'MODULE_PAYMENT_SECUPAY_ORDER_BEFORE_PAYMENT',
            'MODULE_PAYMENT_SECUPAY_SESSION'
        );

        if ($this->conf->secupay_pp_xtc->delivery_differs_disable) {
            $keys[] = 'MODULE_PAYMENT_SPPP_DELIVERY_DISABLE';
        }
    ...
    private function _get_invoice_info($order_id)
    {
        ...
        $secupay_invoice_text = str_replace('[recipient_legal]', utf8_ensure($invoice_data->recipient_legal), MODULE_PAYMENT_SPPP_INVOICE_TEXT_PDF);
        $secupay_invoice_text .= $invoice_data->transfer_payment_data->accountowner;

        $secupay_invoice_account_info = MODULE_PAYMENT_SPPP_KONTO_NR_TITLE . ': ' . utf8_ensure($invoice_data->transfer_payment_data->accountnumber);
        $secupay_invoice_account_info .= ', ' . MODULE_PAYMENT_SPPP_BLZ_TITLE . ': ' . utf8_ensure($invoice_data->transfer_payment_data->bankcode);
        $secupay_invoice_account_info .= ', ' . MODULE_PAYMENT_SPPP_BANKNAME_TITLE . ': ' . utf8_ensure($invoice_data->transfer_payment_data->bankname);

        $secupay_invoice_account_iban = MODULE_PAYMENT_SPPP_IBAN_TITLE . ': ' . utf8_ensure($invoice_data->transfer_payment_data->iban);
        $secupay_invoice_account_iban .= ', ' . MODULE_PAYMENT_SPPP_BIC_TITLE . ': ' . utf8_ensure($invoice_data->transfer_payment_data->bic);

        $secupay_invoice_purpose = MODULE_PAYMENT_SPPP_INVOICE_PURPOSE . ': ' . utf8_ensure($invoice_data->transfer_payment_data->purpose);
        ...
        /*$show_invoice_url = strcmp(MODULE_PAYMENT_SPPP_SHOW_QRCODE, 'Ja') == 0;
        if ($show_invoice_url) {
            $secupay_invoice_url = MODULE_PAYMENT_SPPP_INVOICE_URL_HINT . "\n";
        ...
        */
        ...
        if ($show_due_date) {
            $secupay_invoice_due_date = MODULE_PAYMENT_SPPP_DUE_DATE_TEXT_PDF;
            $invoice_info .= "\n\n";
